...
function App() {
    ...
    useWatch(() => {
        currentAnnot ? modelViewerRef.current.autoRotate = false : modelViewerRef.current.autoRotate = true
    }, [currentAnnot])
    ...
    function calculateOffset (orbit) {
        ...
        xyz[0] = `${parseFloat(xyz[0].replace('rad', '')) + modelViewerRef.current.turntableRotation}rad`
        return xyz.join(' ')
    }
                ...
                <model-viewer bounds="tight"
                    ...
                    {annots.map(annot => (
                        ...
                        >
                            {
                                !annot.page &&
                                <img className='absolute top-0 left-0 transform translate-x-[-20px] translate-y-[-38px] h-[62px] max-w-[48px] w-[48px]' src={annot.brandPinImage} alt="Brand Annot"/>
                            }
